		  ...
		  <?php
			...
			if (!$result) {
				...
				while($rowData = mysqli_fetch_array($result)){
					...
					$sql_establish = "SELECT *
									FROM ESTABLISHES
									WHERE Req_ID = '$reqid'";
					$result_establish = $mysqli->query($sql_establish);
					while($establishData = mysqli_fetch_array($result_establish)){
						echo '<p3 class ="card-text">Connected Since :<br> '.$establishData["Date"].' <br> ID  : '.$_SESSION['user_2'].'</p3>';
